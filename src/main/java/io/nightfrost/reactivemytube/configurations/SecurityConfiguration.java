...
public class SecurityConfiguration {
    ...
    public SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http, AuthenticationWebFilter authenticationWebFilter) {
        return http.csrf(ServerHttpSecurity.CsrfSpec::disable)
            ...
            authorizeExchangeSpec.pathMatchers(HttpMethod.GET, "/api/users/**").hasAnyRole("USER", "ADMIN");
            authorizeExchangeSpec.pathMatchers("/api/admin/**").hasRole("ADMIN");
    ...
    private CorsConfiguration corsConfiguration() {
        ...
        corsConfig.setAllowedOrigins(Arrays.asList("http://localhost:3000", "https://mytube.com")); //FIXME: After domain access we need to reconfigure
